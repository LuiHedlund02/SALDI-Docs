...
This layer should be considered legacy and behavior-heavy.
...
- currently documents address-oriented CRUD in `api/v2/README.md`
...
- appears batch/job oriented rather than request/response oriented
...
This area appears tightly coupled to rental/order creation and booking-related tables.
...
5. **Limited standalone docs** — some behaviors live only in code, especially in `remoteBooking/` and sync scripts.

## Suggested future expansion for this doc
This draft should later be extended with:
- legacy API action catalog
- sync job configuration map
- Dandomain data-flow diagram
- remote booking lifecycle diagram
